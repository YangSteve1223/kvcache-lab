...
The bound explains why pruning can work when the removed tail has low mass, but it also exposes the retrieval problem: if the removed token is the one containing the answer, its contribution is exactly zero regardless of how small $\epsilon$ is.  Mixed precision changes the failure mode from information deletion to bounded approximation error---a qualitatively different and more forgiving regime.
...
Value perturbation is not the only concern.  Quantizing keys perturbs attention logits, which can amplify errors differently depending on the entropy of the attention distribution.  Let $z_j=q^\top k_j/\sqrt{d}$ and $\tilde z_j=z_j+\eta_j$ with $|\eta_j|\le \eta$.  Then the softmax probabilities obey
\begin{equation}
...
The practical implication is that models with local-dominant attention patterns (small $|S|$, low entropy) are more vulnerable to key quantization perturbations than models with broader sink or hybrid patterns.  This explains why Qwen suffers catastrophically despite having higher per-vector cosine similarity under INT4: its attention mass concentrates on a few nearby tokens, so even small key perturbations produce large relative probability shifts that cascade through generation.
...
We state this as a conjecture rather than a theorem because the precise threshold $H^*$ depends on layer-wise interactions that we have not fully characterized.  The empirical evidence---particularly the contrast between Qwen's local-dominant failure and Mistral/Gemma's stability---is consistent with this claim (Section~\ref{sec:ablation} and Section~\ref{sec:discussion}).

\subsection{Mixed-precision error propagation across layers}
\label{sec:layer-error}

The per-layer bound in Eq.~\eqref{eq:quant-bound} treats each layer independently, but in practice KV errors propagate through the residual stream.  We now analyze this propagation.
...
The bound has a useful qualitative implication: if early layers have large perturbations (large $\delta_l$ for small $l$), the error is amplified by all subsequent layers.  Conversely, concentrating the perturbation in later layers yields a smaller amplification factor.  This suggests that per-layer budget allocation---an extension not implemented in the current \method{}---could further improve quality by favoring higher precision in early layers, where errors propagate furthest.
...
The locality measurements in Section~\ref{sec:motivation} establish two facts that shape the policy design: access skew is large enough to justify tiering, and the pattern of skew differs enough across models to require adaptive tier selection.  Figure~\ref{fig:overview} summarizes how \method{} combines these insights into a concrete pipeline.

The prefill worker computes or approximates token importance, pins protected sink tokens, optionally runs a one-time \intiv{} tolerance probe per model, and emits a precision map.  The map determines how each token's KV tensors are quantized before transfer to the decode worker.
...
\begin{equation}
  ...
  \label{eq:importance}
\end{equation}
where $A_j$ is an observed or approximated attention-importance statistic and $\Delta_j=n-j$ is recency distance.  The exponential decay captures the empirical observation that recent tokens receive disproportionately high attention in most Transformer models.  Sink positions are pinned before ranking, so the decay applies only to non-sink tokens.  When attention measurements are unavailable or unreliable, the implementation can fall back to KV-norm scores; in the Qwen decay sweep, this fallback was used and is explicitly reported as such.
...
Having described the \method{} policy, we now evaluate it along three axes: generation quality (PPL and NIAH retrieval), transfer efficiency (TTFT), and robustness (context length, multi-task, seed variance).  This section details the models, hardware, metrics, and baselines; Section~\ref{sec:results} presents the findings.
...
We emphasize that the final measurements use corrected implementations.  Important fixes include using eager attention for locality characterization, replacing zero-out baselines with attention-mask deletion for selection methods, adding the missing \niah{} retrieval question, fixing an adaptive-policy control-flow bug that could expose dropped tokens, and applying the Gemma chat template.  These fixes matter: earlier intermediate logs are marked obsolete and are not used for the main claims.
...
The central question is whether transmitting every token at reduced precision preserves quality better than dropping half the tokens at full precision.  We answer it progressively: first at the reference budget ($\budget=0.5$), then across the full budget sweep, and finally through retrieval, latency, and robustness analyses.

\subsection{PPL at the 50\pct{} transfer budget}
\label{sec:rq7}

At $\budget=0.5$, \method{} Greedy reduces to all-token \intviii{} transfer, which is nearly lossless for all three models (Table~\ref{tab:ppl50}).  The comparison with selection baselines is stark: while PDTrim degrades PPL by +25.85\pct{} on Qwen, +22.07\pct{} on Mistral, and +35.63\pct{} on Gemma, \method{} Greedy changes PPL by only +1.97\pct{}, $-$0.06\pct{}, and $-$0.44\pct{}, respectively.  SinkProtect further improves Qwen (+1.39\pct{}) and Gemma ($-$0.47\pct{}) at the same budget by explicitly pinning sink tokens to \fp{}.
...
\end{axis}
\end{tikzpicture}
\caption{At the same $\budget=0.5$ transfer budget, mixed precision preserves PPL far better than binary selection.  The catastrophic Balanced/Qwen value is omitted from the plot scale and shown in Table~\ref{tab:ppl50}.}
\label{fig:ppl50}
\end{figure}

The Balanced row is intentionally not hidden.  Surprisingly, assigning substantial \intiv{} mass \emph{uniformly} across all positions---ignoring importance entirely---causes Qwen PPL to explode by +7506.84\pct{}, even though the same budget is only mildly harmful for Mistral (+2.71\pct{}) and Gemma (+2.30\pct{}).  The reason is that Qwen's local-dominant attention amplifies even small key perturbations through softmax normalization (Section~\ref{sec:softmax-amp}), so placing INT4 on high-attention local tokens is catastrophic.  For INT4-tolerant models, the Balanced penalty is small because INT4 is safe regardless of position.  This asymmetry motivates the probe.

\subsection{Budget sweep}
\label{sec:rq8}

As the budget drops below 0.5, INT4 tokens begin to appear and the advantage of mixed precision over selection widens (Table~\ref{tab:budget}).  For Mistral and Gemma, 3-tier \method{} remains usable at $\budget=0.3$ (+5.41\pct{} and +2.45\pct{} PPL), while PDTrim and SWS degrade much more severely.  Qwen, however, cannot safely use the 3-tier policy at $\budget=0.3$ because roughly 80\pct{} of tokens would be assigned \intiv{}, triggering the catastrophic failure described above.  The correct Qwen policy at this budget is adaptive 2-tier; we evaluate it primarily through retrieval because the main PPL scan focused on fixed 3-tier variants.
...
The full PPL-vs-budget picture across all 11 budget points (Figure~\ref{fig:ppl-sweep}) confirms and extends the single-budget findings.  For Mistral and Gemma, \method{}\_Greedy and Random\_Tier lie far below PDTrim and SWS across the entire operating range, and the improvement is largest at low budgets where INT4 tokens carry useful information that would otherwise be lost entirely.  For Qwen, the safe operating range starts at $\budget=0.5$: below this threshold, the 3-tier policy assigns INT4 to a large fraction of tokens and quality collapses.  The shaded unsafe zone in the Qwen subplot marks this region.

Surprisingly, Random\_Tier and \method{}\_Greedy converge at $\budget=0.5$ for all models---because at that budget, all tokens receive INT8 under both policies, rendering importance ranking irrelevant.  Below $\budget=0.5$, the two diverge, but not always in the expected direction.  Counter-intuitively, at $\budget=0.3$ on Mistral, Random\_Tier (4.91\pct{}) slightly outperforms Greedy (5.41\pct{}).  We trace this anomaly to the interaction between INT4 placement and attention structure in Section~\ref{sec:ablation-ranking}.
...
Retrieval is where the difference between approximation and deletion becomes most visible.  A pruned needle cannot be recovered by the decode worker unless the system fetches the missing KV; a low-precision needle, by contrast, still contributes an approximate signal that can guide generation toward the correct answer.

That theoretical advantage translates into a large empirical gap (Table~\ref{tab:niah}).  At the aggressive $\budget=0.3$, \method{} adaptive reaches 52.6\pct{} NIAH accuracy on Qwen versus PDTrim's 26.3\pct{}, and Mistral and Gemma preserve retrieval at or near 100\pct{} under the 3-tier policy.  By $\budget=0.5$, all three models reach 100\pct{} under \method{}, while PDTrim remains below 50\pct{}.
...
\end{axis}
\end{tikzpicture}
\caption{At the aggressive $\budget=0.3$ budget, \method{} preserves retrieval substantially better than keep/drop baselines.  The Mistral bar uses the adaptive verification prompt format; the raw scan is 89.5\pct{}, equal to its FP16 baseline under that prompt.}
\label{fig:niah03}
\end{figure}
...
The aggregate \niah{} accuracy above obscures the depth-dependent structure of retrieval success.  Decomposing accuracy by needle insertion depth reveals a striking pattern (Figure~\ref{fig:niah-heatmap}): \method{} exhibits a characteristic U-shaped retrieval curve at low budgets, while PDTrim shows a monotone depth threshold.
...
The heatmap reveals a characteristic U-shaped retrieval pattern for \method{} at low budgets because of how importance scoring allocates precision.  At $\budget=0.3$, Adaptive successfully retrieves needles at shallow depths ($d \le 0.35$, near the attention sink region) and at deep depths ($d \ge 0.85$, near the local window), while failing in the middle range ($0.40 \le d \le 0.80$) where the budget cannot cover enough mid-context positions with adequate precision.

PDTrim, by contrast, shows a monotone depth threshold: retrieval succeeds only when the needle falls within the retained suffix.  At $\budget=0.3$, only depths $d \ge 0.75$ are retrieved.  The U-shape is absent because PDTrim deletes the early context entirely; there is no approximate representation to exploit, so any needle outside the retained window is simply invisible.
...
Quality improvements are only useful if they come with actual transfer savings.  Table~\ref{tab:latency} confirms that reducing the KV payload at $\budget=0.5$ cuts TTFT by 50--62\pct{} across all tested model/length pairs.  Decode throughput is mostly stable and can improve at longer lengths, where reducing KV traffic relieves memory-bandwidth pressure.
...
These timings isolate the transfer path; they do not constitute a full production serving study.  A complete evaluation would need to include a PD scheduler, continuous batching, network contention, and cold-tier fetch policies.

\subsection{Context length}
\label{sec:rq10}

The results so far use a fixed 2048-token context.  As context grows, the KV payload expands linearly, making compression more valuable---but also raising the risk that accumulated approximation error degrades quality.  Table~\ref{tab:context} shows that \method{} remains close to baseline across 2K--8K contexts.  Mistral is especially stable, with absolute PPL deltas within 0.3\pct{} for Greedy.  Gemma 8K was not completed because of OOM risk in the available setup.
...
Table~\ref{tab:probe} shows the probe decisions.  The probe is deliberately small---just three NIAH trials---yet it catches the only catastrophic \intiv{} failure in the evaluated set.  Surprisingly, Table~\ref{tab:cosine} reveals that Qwen's average \intiv{} key cosine similarity is \emph{higher} than Mistral's and Gemma's, and its value cosine (0.9895) is the highest of the three.  The reason cosine alone cannot predict INT4 safety is that it measures per-vector approximation quality, which ignores how perturbations propagate through a low-entropy attention distribution (Section~\ref{sec:softmax-amp}).  A vector-level statistic, no matter how high, cannot capture the distribution-level amplification that causes Qwen's failure.
...
The aggregate cosine similarity in Table~\ref{tab:cosine} masks significant per-layer variation.  Decomposing by layer (Figure~\ref{fig:per-layer-cosine}) reveals three patterns that carry design implications.

First, value cosine similarity is consistently higher than key cosine across all layers and all models, confirming that key quantization is the dominant source of error---because key perturbations affect the attention distribution itself, while value perturbations only affect the weighted sum.  Second, the first and last layers tend to have higher key cosine similarity than middle layers, particularly for Qwen (layer 0: 0.9947, layer 27: 0.9955) and Mistral (layer 0: 0.9896, layer 35: 0.9912).  Third, Gemma exhibits the widest per-layer variance in key cosine, ranging from 0.9334 (layer 1) to 0.9798 (layer 18).  This variance suggests that a per-layer budget allocation---giving more precision to low-cosine layers---could yield further improvements, but we leave this extension to future work.
...
The main results establish that \method{} preserves PPL and retrieval better than selection baselines.  We now probe the robustness of this conclusion: how sensitive are the results to PDTrim's hyperparameters, to different task domains, to random seed choice, and to each of \method{}'s design decisions?

\subsection{PDTrim first-ratio sensitivity}
PDTrim has a first-ratio hyperparameter that determines how much of the beginning of the sequence is retained.  Table~\ref{tab:fr} shows that the best value differs by model---Qwen prefers 0.3, Mistral prefers 0.7, and Gemma prefers 0.9---because each model has a different attention pattern.  A fixed first/last split is therefore fragile across attention patterns.
...
Table~\ref{tab:tasks} reports a small multi-task scan at $\budget=0.5$.  \method{} is consistently better on WikiText and code.  Science text is harder for both methods, likely because domain-specific workloads have more diffuse attention or more brittle long-range dependencies---the same low-precision approximation that preserves WikiText fluency may not sustain the precise factual recall that science passages demand.  This is a limitation, not a reason to hide the result; it points to the need for cold-tier fetches or task-aware budgets.
...
Two findings emerge.  First, SinkProtect provides a small but consistent improvement over Greedy on Qwen (+1.39\pct{} vs.\ +1.97\pct{}) and Gemma ($-$0.47\pct{} vs.\ $-$0.44\pct{}), while being slightly worse on Mistral ($-$0.10\pct{} vs.\ $-$0.06\pct{}).  The improvement is larger for models whose importance scores do not already rank sinks at the top: Qwen and Gemma have hybrid or local patterns where sinks might not score highest by attention mass alone, so pinning them explicitly helps.  Mistral's sink-dominant pattern naturally places sinks at the top of the importance ranking, making explicit pinning redundant.

Second, Balanced is catastrophic for Qwen (+7506.84\pct{}) but only mildly harmful for Mistral (+2.71\pct{}) and Gemma (+2.30\pct{}).  The reason is not the lack of sink protection per se, but the uniform distribution of INT4 tiers across all positions: this places INT4 on high-attention local tokens, which Qwen's low-entropy attention amplifies through softmax normalization.  For INT4-tolerant models (Mistral, Gemma), the Balanced penalty is small because INT4 is safe regardless of which positions receive it; for INT4-intolerant models (Qwen), placing INT4 on any significant fraction of tokens is unsafe.

\subsubsection{Importance ranking vs.\ random assignment}
\label{sec:ablation-ranking}

The budget-sweep curves in Figure~\ref{fig:ppl-sweep} already hint at the relationship between Greedy and Random\_Tier.  At $\budget=0.5$, the two are identical because all tokens receive INT8 under both policies.  The more interesting question is what happens below $\budget=0.5$, where INT4 tokens appear and the assignment matters (Table~\ref{tab:ablation-ranking}).
...
Counter-intuitively, Random\_Tier is not consistently worse than Greedy, and in some cases it is slightly better.  At $\budget=0.30$, Mistral Random\_Tier gives +4.91\pct{} vs.\ Greedy's +5.41\pct{}; at $\budget=0.35$, Mistral Random\_Tier gives +3.07\pct{} vs.\ +4.05\pct{}.  The reason is that Greedy assigns INT4 to the lowest-importance tokens, which are typically clustered in the middle of the context.  If those middle positions happen to be queried by certain attention heads---a scenario not captured by the aggregate importance score---concentrating low precision there is harmful.  Random\_Tier, by chance, may distribute INT4 positions more evenly across the sequence, avoiding a concentrated low-precision region.
...
The preceding ablations address incremental design choices.  The most consequential one, however, is the INT4 tolerance probe: it determines whether the policy has access to the cheapest tier at all.  Table~\ref{tab:ablation-probe} compares the 3-tier Greedy policy (which enables INT4) with the adaptive 2-tier policy (which disables INT4 after the probe fails) for Qwen at $\budget=0.3$.
...
If per-token precision allocation helps, one might ask: when does it help \emph{beyond} what uniform quantization already provides?  Table~\ref{tab:uniform} compares the two approaches to delineate exactly where \method{}'s advantage emerges.
...
The advantage of \method{} emerges at two points.  First, at non-standard budgets ($\budget \ne 0.25, 0.5$), uniform quantization cannot exactly match the budget because uniform INT8 corresponds to $\budget=0.5$ and uniform INT4 corresponds to $\budget=0.25$.  There is no uniform scheme for $\budget=0.3$ or $\budget=0.75$ without mixing precisions, and \method{} provides a principled way to mix.  Second, at $\budget=0.25$, the question becomes which tokens should receive INT4 and which should receive INT8.  Uniform\_INT4 gives INT4 to everything; \method{}\_Greedy gives INT8 to the 20\pct{} highest-importance tokens and INT4 to the rest.  For INT4-tolerant models (Mistral, Gemma), both approaches yield the same result because the Greedy assignment also places INT4 on the vast majority of tokens.  For INT4-intolerant models (Qwen), both fail catastrophically---which is precisely why the probe matters.
