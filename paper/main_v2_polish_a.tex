...
\begin{enumerate}[leftmargin=1.6em,itemsep=2pt]
    ...
    \item \method{}, a mixed-precision policy with sink protection, importance-ranked tier assignment, and a lightweight \intiv{} tolerance probe, is proposed.
    \item GPU-verified results on Qwen2.5-7B-Instruct, Mistral-7B-Instruct-v0.3, and Gemma-2-9B-it cover PPL, \niah{} retrieval, TTFT/TPS timing, context length, multi-task robustness, and quantization error.
    ...
    \item A systematic ablation study isolates the contributions of sink protection, importance ranking, and the INT4 tolerance probe, while the theoretical framework is extended with softmax amplification bounds and mixed-precision error propagation analysis.
\end{enumerate}
...
\subsection{Overview}
Figure~\ref{fig:overview} summarizes \method{}.  The prefill worker computes or approximates token importance, pins protected sink tokens, optionally runs a one-time \intiv{} tolerance probe per model, and emits a precision map.  The map determines how each token's KV tensors are quantized before transfer to the decode worker.  Not all models, however, can safely tolerate aggressive quantization at the lowest tier---a question addressed by the INT4 tolerance probe.
...
\method{} runs a probe once per model.  The probe performs three \niah{} trials under the aggressive 3-tier greedy configuration at $\budget=0.3$.  If at least two trials succeed, \intiv{} is enabled.  Otherwise the model falls back to 2-tier \fp{}+\intviii{}.  In the experiments, Qwen2.5-7B fails the probe (0/3), while Mistral-7B and Gemma-2-9B pass (3/3 each).  The probe is deliberately task-relevant: it asks whether aggressive low-tier KV still supports retrieval, which is where token dropping and unsafe quantization are most visible.
...
\paragraph{Models.}
The evaluation covers Qwen2.5-7B-Instruct, Mistral-7B-Instruct-v0.3, and Gemma-2-9B-it.  The locality characterization also includes Qwen2.5-14B, but the full quality suite is limited to the three 7B--9B models.
...
The final measurements use corrected implementations.  Important fixes include using eager attention for locality characterization, replacing zero-out baselines with attention-mask deletion for selection methods, and adding the missing \niah{} retrieval question.  Further corrections addressed an adaptive-policy control-flow bug that could expose dropped tokens and the Gemma chat template.  These fixes matter: earlier intermediate logs were marked obsolete and are not used for the main claims.
...
The aggregate cosine similarity in Table~\ref{tab:cosine} masks significant per-layer variation.  Figure~\ref{fig:per-layer-cosine} shows the INT4 key and value cosine similarity for each layer of the three models, extracted from the experiment data.
...
Nor does this finding invalidate the exchange argument of Proposition~1, which guarantees optimality for a fixed importance score.  Rather, it suggests that the aggregate attention-based importance score is an imperfect proxy for actual downstream quality, especially when INT4 tokens are present.  A more refined scoring function---perhaps incorporating per-head or per-layer importance---could close this gap, but this extension is left to future work.
...
At $\budget=0.5$, Uniform\_INT8 and \method{}\_Greedy produce identical results because both assign INT8 to all tokens.  As expected, when the budget equals the cost of uniform INT8, there is no room for per-token differentiation.
...
\textbf{Model coverage.}  The full GPU quality suite covers only three 7B--9B models, and larger models may shift the \intiv{} tolerance boundary.  The Qwen2.5-14B locality data is promising but incomplete.
...
StreamingLLM identifies attention sinks~\citep{streamingllm}.  H$_2$O, SnapKV, PyramidKV, and DynamicKV select or compress KV according to heavy-hitter or task-aware signals~\citep{h2o,snapkv,pyramidkv,dynamickv}.  These methods are primarily selection-oriented.  Why stop at keeping or dropping a token when its representation can instead be degraded gracefully?  \method{} retains the importance idea but changes the action from keep/drop to precision allocation.
...
PD disaggregation makes KV cache a communication object.  Existing keep/drop methods reduce transfer volume but lose all information from dropped tokens.  Should PD KV transfer be viewed solely as a pruning problem?  \method{} replaces that binary decision with a per-token precision spectrum.  It protects high-importance tokens at \fp{}, sends medium-importance tokens at \intviii{}, and uses \intiv{} for low-importance tokens only when a lightweight probe says the model tolerates it.
