...
PD disaggregation is becoming a standard serving pattern because prefill is parallel and compute-heavy while decode is sequential and latency-sensitive.  Systems such as Splitwise, DistServe, and Mooncake split the two phases across different workers for independent scheduling and scaling~\citep{splitwise,distserve,mooncake}.  The price of this separation is that the KV cache produced during prefill must be made available to decode.  In long-context serving, that cache is large enough for transfer bandwidth and transfer latency to become first-order concerns.

A common response is to transmit fewer tokens.  StreamingLLM identifies attention sinks and shows that the beginning of the sequence can remain important for stable generation~\citep{streamingllm}; H$_2$O, SnapKV, PyramidKV, and related methods select or compress tokens according to attention-derived importance~\citep{h2o,snapkv,pyramidkv,dynamickv}.  PDTrim adapts this keep/drop view to PD transfer by retaining selected first/last regions and pruning the rest~\citep{pdtrim}.  The binary decision is unnecessarily harsh in the PD setting: a token slightly below the pruning threshold carries no information after being dropped, even if a reduced-precision copy would have been sufficient.

\method{} assigns every token a precision level.  The policy assigns \fp{} to tokens whose perturbation would be most harmful, \intviii{} to medium-importance tokens, and \intiv{} to low-importance tokens when the model tolerates \intiv{}.  Unlike uniform \intviii{}, which treats all positions equally, \method{} reserves high fidelity for attention sinks, recent tokens, or other positions with high estimated contribution to decode.  Unlike pruning, the low tier still transmits information, which matters especially for retrieval tasks where the needle may appear outside a selected window.

\intiv{} is not a universally safe low tier.  Qwen2.5-7B is local-dominant and extremely sensitive to \intiv{} KV perturbations: a balanced 3-tier assignment at $\budget=0.5$ increases PPL by +7506.84\pct{}.  Mistral-7B and Gemma-2-9B tolerate \intiv{} in low-importance positions with small PPL changes and strong retrieval preservation.  \method{} therefore includes a probe that decides whether to enable \intiv{} for each model.
...
\begin{equation}
  ...
  \label{eq:budget}
\end{equation}
where $\pi$ denotes the precision assignment.  A budget of $\budget=0.5$ corresponds to sending all tokens at \intviii{}, or to any mixture with the same average byte cost.  At $\budget=0.5$, several policies collapse to the same all-\intviii{} behavior because \intviii{} is nearly lossless in our measurements.
...
\end{equation}
The error contribution of a tier scales with the \emph{attention mass assigned to that tier}, not merely with the number of tokens in the tier.  It motivates sending high-attention tokens at high precision and low-attention tokens at lower precision.
...
The bound explains why pruning can work when the removed tail has low mass, but it also exposes the retrieval problem: if the removed token contains the answer, its contribution is exactly zero.  Mixed precision changes the failure mode from information deletion to approximation error.
...
This justifies the core design: once a score is chosen, high-importance tokens should receive at least as much precision as low-importance tokens.
...
Models with local-dominant attention patterns (small $|S|$, low entropy) are more vulnerable to key quantization perturbations than models with broader sink or hybrid patterns.  Qwen, despite having higher per-vector cosine similarity under INT4, suffers catastrophically because its attention mass concentrates on a few nearby tokens, so even small key perturbations produce large relative probability shifts.
...
The per-layer bound in Eq.~\eqref{eq:quant-bound} treats each layer independently, but KV errors propagate through the residual stream.  Consider an $L$-layer Transformer.  Let the output of layer $l$ be $x^{(l)} = x^{(l-1)} + a^{(l)}$, where $a^{(l)}$ is the attention output.  If the KV cache at layer $l$ has perturbation $\delta_l$ (from Eq.~\eqref{eq:quant-bound}), the perturbed attention output satisfies $\|\tilde a^{(l)} - a^{(l)}\| \le \delta_l$.  Then:
...
Early-layer perturbations (large $\delta_l$ for small $l$) are amplified by all subsequent layers, while later-layer perturbations have smaller amplification factors.  This suggests that per-layer budget allocation---an extension not implemented in the current \method{}---could further improve quality by favoring higher precision in early layers.
...
\subsection{Overview}
The prefill worker computes or approximates token importance, pins protected sink tokens, optionally runs a one-time \intiv{} tolerance probe per model, and emits a precision map (Figure~\ref{fig:overview}).  The map determines how each token's KV tensors are quantized before transfer to the decode worker.
...
Figure~\ref{fig:sysarch} places \method{} within a complete PD disaggregated serving system.  Importance scoring and quantization happen at the prefill worker \emph{before} network transfer, while dequantization and attention happen at the decode worker \emph{after} receipt.  This placement ensures that the network payload is already compressed, and the decode worker only needs lightweight dequantization (table lookup) before running attention.
...
\end{equation}
Negative values are not interpreted as quality improvements (see Section~\ref{sec:discussion}).
...
At $\budget=0.5$, \method{} Greedy is equivalent to all-token \intviii{} transfer, so Greedy, Uniform\_INT8, and Random\_Tier produce the same PPL.  PDTrim degrades PPL by +25.85\pct{} on Qwen, +22.07\pct{} on Mistral, and +35.63\pct{} on Gemma; \method{} Greedy changes PPL by +1.97\pct{}, $-$0.06\pct{}, and $-$0.44\pct{}.  SinkProtect slightly improves Qwen (+1.39\pct{}) and Gemma ($-$0.47\pct{}) at the same budget.
...
The Balanced row assigns substantial \intiv{} mass even at $\budget=0.5$ and causes Qwen PPL to explode.  This is not a plotting artifact or a malformed cache write; it is reproduced by direct \intiv{} controls and motivates the probe.

\subsection{Budget sweep}
\label{sec:rq8}

Table~\ref{tab:budget} shows the budget sweep.  For Mistral and Gemma, 3-tier \method{} remains usable at $\budget=0.3$ (+5.41\pct{} and +2.45\pct{} PPL), while PDTrim and SWS degrade by 31--82\pct{}.  Qwen is marked unsafe for 3-tier at $\budget=0.3$ because the assignment places roughly 80\pct{} of tokens in \intiv{}, producing catastrophic PPL.  The correct Qwen policy at this budget is adaptive 2-tier; we evaluate it primarily through retrieval.
...
Figure~\ref{fig:ppl-sweep} presents the PPL-vs-budget curves across all 11 budget points.  For Mistral and Gemma, \method{}\_Greedy and Random\_Tier lie far below PDTrim and SWS across all budgets, with the largest gap at low budgets where INT4 tokens carry information that would otherwise be lost entirely.  For Qwen, the safe operating range starts at $\budget=0.5$: below this threshold, the 3-tier policy assigns INT4 to a large fraction of tokens and quality collapses.  The shaded unsafe zone in the Qwen subplot marks this region.

At $\budget=0.5$, Random\_Tier and \method{}\_Greedy converge because both assign all tokens to INT8.  Below $\budget=0.5$, the two diverge, but not always in the expected direction---for Mistral at $\budget=0.30$, Random\_Tier (4.91\pct{}) is slightly better than Greedy (5.41\pct{}).  We analyze this counterintuitive result in Section~\ref{sec:ablation-ranking}.
...
Retrieval is where approximation versus deletion is most visible.  A pruned needle contributes nothing; a low-precision needle still provides an approximate signal.
...
The aggregate \niah{} accuracy in Table~\ref{tab:niah} obscures the depth-dependent structure.  Figure~\ref{fig:niah-heatmap} shows the full depth-vs-budget retrieval map for Qwen2.5-7B.
...
\method{} at low budgets shows a characteristic U-shaped retrieval pattern.  At $\budget=0.3$, Adaptive successfully retrieves needles at shallow depths ($d \le 0.35$, near the attention sink region) and at deep depths ($d \ge 0.85$, near the local window), while failing in the middle range ($0.40 \le d \le 0.80$).  This U-shape reflects the importance scoring: sink tokens and recent tokens receive FP16 or INT8, while the budget is insufficient to cover all mid-context positions with adequate precision.

PDTrim shows a monotone depth threshold: at $\budget=0.3$, only depths $d \ge 0.75$ are retrieved.  The U-shape is absent because PDTrim simply deletes the early context entirely.

\subsection{Latency evidence}
\label{sec:rq9}

At $\budget=0.5$, TTFT falls by 50--62\pct{} across all tested model/length pairs (Table~\ref{tab:latency}).  Decode throughput is mostly stable and can improve at longer lengths, where reducing KV traffic relieves memory-bandwidth pressure (e.g., Gemma-2-9B at 4K: +33\pct{} TPS).
...
These timings isolate the transfer path; they do not include a complete PD scheduler, continuous batching, network contention, or cold-tier fetch policies (see Section~\ref{sec:limitations}).

\subsection{Context length}
\label{sec:rq10}

\method{} remains close to baseline across 2K--8K contexts (Table~\ref{tab:context}).  Mistral is especially stable, with absolute PPL deltas within 0.3\pct{} for Greedy.  Gemma 8K was not completed because of OOM risk in the available setup.
...
Table~\ref{tab:probe} shows the probe decisions.  Table~\ref{tab:cosine} shows why a separate probe is necessary: Qwen's average \intiv{} key cosine similarity (0.9770) is \emph{higher} than Mistral's (0.9792) and Gemma's (0.9636), yet Qwen fails catastrophically under INT4.  A vector-level cosine statistic cannot predict how key perturbations propagate through a low-entropy attention distribution (Section~\ref{sec:softmax-amp}).
...
PDTrim has a first-ratio hyperparameter that determines how much of the beginning of the sequence is retained.  The best value differs by model (Table~\ref{tab:fr}): Qwen prefers 0.3, Mistral prefers 0.7, and Gemma prefers 0.9.  A fixed first/last split is therefore fragile across attention patterns.
...
Table~\ref{tab:tasks} reports a multi-task scan at $\budget=0.5$.  \method{} is consistently better on WikiText and code; Science text is harder for both methods (Mistral: +33.9\pct{} vs.\ PDTrim's +152.8\pct{}), suggesting that domain-specific workloads may have more diffuse attention or more brittle long-range dependencies.
...
We isolate the contribution of sink token protection, importance-based ranking, and the INT4 tolerance probe.
...
Balanced is catastrophic for Qwen (+7506.84\pct{}) but only mildly harmful for Mistral (+2.71\pct{}) and Gemma (+2.30\pct{}).  The Qwen catastrophe is not caused by the lack of sink protection per se, but by the uniform distribution of INT4 tiers across all positions, which places INT4 on high-attention local tokens.  For INT4-tolerant models, the Balanced penalty is small because INT4 is safe even on high-attention positions; for INT4-intolerant models, the penalty is catastrophic because INT4 on any significant fraction of tokens is unsafe.

\subsubsection{Importance ranking vs.\ random assignment}
\label{sec:ablation-ranking}

At $\budget=0.5$, Greedy and Random\_Tier are identical because all tokens receive INT8 under both policies.  Below $\budget=0.5$, INT4 tokens appear and the assignment matters.
...
Random\_Tier is not consistently worse than Greedy, and in some cases is slightly better.  At $\budget=0.30$, Mistral Random\_Tier gives +4.91\pct{} vs.\ Greedy's +5.41\pct{}; at $\budget=0.35$, Mistral Random\_Tier gives +3.07\pct{} vs.\ +4.05\pct{}.  For Gemma at $\budget=0.35$, Random\_Tier (+1.87\pct{}) is slightly worse than Greedy (+1.53\pct{}).

The explanation lies in the interaction between INT4 placement and attention structure.  Greedy assigns INT4 to the lowest-importance tokens, which are typically in the middle of the context.  If those middle positions happen to be queried by certain attention heads (a scenario not captured by the aggregate importance score), placing INT4 there can be harmful.  Random\_Tier, by chance, may distribute INT4 positions more evenly, avoiding concentration of low precision in any single functional region.

This finding does not invalidate the exchange argument of Proposition~1, which guarantees optimality for a fixed importance score.  Rather, it suggests that the aggregate attention-based importance score is an imperfect proxy for actual downstream quality when INT4 tokens are present.  A more refined scoring function---incorporating per-head or per-layer importance---could close this gap, but we leave this to future work.

\subsubsection{2-tier vs.\ 3-tier: the probe is necessary}
\label{sec:ablation-probe}

Table~\ref{tab:ablation-probe} compares the 3-tier Greedy policy with the adaptive 2-tier policy for Qwen at $\budget=0.3$.  The probe is the difference between catastrophic failure (4.2\pct{} NIAH) and usable quality (52.6\pct{} NIAH).  Without the probe, deploying a 3-tier policy on Qwen would destroy both PPL and retrieval.  The probe adds negligible overhead---three NIAH trials at prefill time, run once per model---but prevents the most severe failure mode in this study.
...
At $\budget=0.5$, Uniform\_INT8 and \method{}\_Greedy produce identical results because both assign INT8 to all tokens.  The advantage of \method{} emerges at non-standard budgets ($\budget \ne 0.25, 0.5$), where uniform quantization cannot exactly match the budget and \method{} provides a principled way to mix precisions.  At $\budget=0.25$, \method{}\_Greedy gives INT8 to 20\pct{} of tokens (the highest-importance ones) and INT4 to the rest; for INT4-tolerant models this matches Uniform\_INT4, while for INT4-intolerant models both fail catastrophically---which is why the probe matters.
...
Raw cosine similarity alone cannot predict INT4 safety.  Across the three models, \intiv{} key cosine values are closely clustered (0.9770--0.9792), and Qwen's value cosine (0.9895) is actually the highest of the three.  Yet Qwen's downstream PPL explodes under INT4 while the others remain stable.  The mechanism, formalized in Section~\ref{sec:softmax-amp}, is softmax amplification under local-dominant attention.
...
\paragraph{Complementarity with uniform KV quantization.}
...
Denning's working-set model describes the active memory footprint of a process~\citep{denning1968working}.  Unlike Denning's model, which evicts cold pages entirely, \method{} retains cold tokens at reduced precision rather than dropping them, preserving approximate information for retrieval tasks where the ``cold'' page may contain the answer.
...
PD disaggregation makes KV cache a communication object.  Existing keep/drop methods reduce transfer volume but lose all information from dropped tokens.  \method{} replaces that binary decision with a per-token precision spectrum: \fp{} for high-importance tokens, \intviii{} for medium-importance tokens, and \intiv{} for low-importance tokens only when a lightweight probe says the model tolerates it.

Three claims follow from the data.  (1)~Mixed precision preserves PPL far better than binary selection at the same budget: at $\budget=0.5$, \method{} changes PPL by at most +1.97\pct{} (Qwen) while PDTrim degrades by 22--36\pct{}.  (2)~Retaining all tokens at some precision preserves retrieval: at $\budget=0.3$, adaptive \method{} reaches 52.6\pct{} NIAH on Qwen versus PDTrim's 26.3\pct{}, and Mistral/Gemma preserve retrieval under the 3-tier policy.  (3)~\intiv{} tolerance is model-dependent and cannot be inferred from cosine similarity alone; empirical probing is necessary for safe deployment.
