#!/usr/bin/env python3
"""
NIAH Depth Scan 8K: Extended context test
==========================================
Test needle retrieval at 8192 tokens (Gemma) and 4096 (Mistral SWA limit).
Fix: isalnum() bug — strip punctuation before matching.
"""
...
MODELS = {
    "mistral7b": {"path": "/root/autodl-tmp/Mistral-7B-Instruct-v0.3", "seq_len": 4096},
    "gemma9b": {"path": "/root/autodl-tmp/gemma-2-9b-it", "seq_len": 8192},
}
...
def check_niah_retrieval(model, tok, input_ids, needle_text, strategy, budget, 
    ...
    try:
        ...
        # BUG FIX: strip punctuation before isalnum()
...
def main():
    log("="*60)
    log("NIAH Depth Scan 8K - Extended Context Test")
    log("Fix: isalnum() bug (needle punctuation stripped)")
    log("="*60)
    ...
    for name, cfg in MODELS.items():
        ...
        out_file = os.path.join(OUTPUT_DIR, f"niah_depth_scan_8k_{name}.json")
        if os.path.exists(out_file):
            log(f"SKIP {name}: output already exists at {out_file}")
            continue
        
        log(f"\nProcessing: {name} (seq_len={seq_len})")
        try:
