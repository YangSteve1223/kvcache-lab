#!/usr/bin/env python3
"""
NIAH Depth Scan补充: Mistral 8K + Gemma 4K
===========================================
补齐两个模型各缺的context长度数据。
"""
...
MODELS = {
    "mistral7b": {"path": "/root/autodl-tmp/Mistral-7B-Instruct-v0.3", "seq_len": 8192},
    "gemma9b": {"path": "/root/autodl-tmp/gemma-2-9b-it", "seq_len": 4096},
}
...
def check_niah_retrieval(model, tok, input_ids, needle_text, strategy, budget, 
    ...
    try:
        ...
        # Fixed: strip punctuation before isalnum()
...
def main():
    log("="*60)
    log("NIAH Depth Scan补充: Mistral 8K + Gemma 4K")
    log("="*60)
