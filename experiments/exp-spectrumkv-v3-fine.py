#!/usr/bin/env python3
"""
SpectrumKV Simulation v3 (Fine-Grained)
========================================
Fine-grained budget (step 0.05) and depth (step 0.05) scanning.
...
SEQ_LENS = [2048, 4096]
BUDGETS = [0.25, 0.30, 0.35, 0.40, 0.45, 0.50, 0.55, 0.60, 0.65, 0.70, 0.75]
DEPTH_FRACS = [0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95]
SINK = 4
N_RUNS = 10
...
RESULTS_PATH = "experiment_logs/spectrumkv_v3_fine.json"
...
def generate_plots(all_results):
    ...
    for idx in range(len(BUDGETS), len(axes_flat)):
        axes_flat[idx].set_visible(False)
    
    plt.suptitle('NIAH by Budget: Selection vs SpectrumKV (v3 Fine-Grained)', fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURES_DIR, 'fig_spectrumkv_v3_fine_niah_by_budget.pdf'), dpi=150, bbox_inches='tight')
    plt.savefig(os.path.join(FIGURES_DIR, 'fig_spectrumkv_v3_fine_niah_by_budget.png'), dpi=150, bbox_inches='tight')
    plt.close()
    ...
    # Show a few representative budgets instead of all 11
    plot_budgets = [0.3, 0.5, 0.7]
    fig, axes = plt.subplots(1, 3, figsize=(18, 6))
    
    key_methods = ["PDTrim", "SpectrumKV_Greedy_AttnOnly", "SpectrumKV_Greedy_ValueAware"]
    depth_labels = [f'{int(d*100)}%' for d in DEPTH_FRACS]
    
    for idx, budget in enumerate(plot_budgets):
        ax = axes[idx]
    ...
    plt.suptitle('NIAH by Depth: PDTrim vs SpectrumKV (v3 Fine-Grained)', fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURES_DIR, 'fig_spectrumkv_v3_fine_depth_niah.pdf'), dpi=150, bbox_inches='tight')
    plt.savefig(os.path.join(FIGURES_DIR, 'fig_spectrumkv_v3_fine_depth_niah.png'), dpi=150, bbox_inches='tight')
    plt.close()
